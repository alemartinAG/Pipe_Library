...
public class PetriNetView extends Observable implements Cloneable, IObserver, Serializable {
	...
	public void setTokenViews(LinkedList<TokenView> tokenViews) {
		if (this._tokenViews == null) {
			...
			for (int i = 0; i < tokenViews.size(); i++) {
				...
				if (i < currentSize) {
					this._tokenViews.get(i).setColor(tokenViews.get(i).getColor());
				...
				if (this._tokenViews.get(i).isEnabled()) {
					for (Object p : _placeViews) {
						int pos = positionInTheList(this._tokenViews.get(i).getID(),
								((PlaceView) p).getCurrentMarkingView());
						if (pos == -1) {
							((PlaceView) p).getCurrentMarkingView()
									.add(new MarkingView(this._tokenViews.get(i), 0 + ""));
						}
					}
	...
	private TransitionView createTransition(Element element) {
		...
		TransitionView transitionView = new TransitionView(positionXInput, positionYInput, idInput, nameInput,
				nameOffsetXInput, nameOffsetYInput, timedTransition, infiniteServer, angle,
	...
	private PlaceView createPlace(Element element) {
		...
		if (initialMarkingTempStorage.length() > 0) {
			...
			if (stringArray.length == 1) {
				if (getActiveTokenView() == null) {
					...
					MarkingView markingView = new MarkingView(getActiveTokenView(),
		...
		return new PlaceView(positionXInput, positionYInput, idInput, nameInput, nameOffsetXInput, nameOffsetYInput,
	...
	public void storeCurrentMarking() {
		...
		for (int placeNo = 0; placeNo < placeSize; placeNo++) {
			_markingVectorAnimationStorage[placeNo] = Copier
					.mediumCopy(_placeViews.get(placeNo).getCurrentMarkingView());
		}
	}
			...
			if (transitionView.isEnabled() && _placeViews != null) {
				...
				for (int placeNo = 0; placeNo < _placeViews.size(); placeNo++) {
					for (MarkingView markingView : _placeViews.get(placeNo).getCurrentMarkingView()) {
						...
						int oldMarkingPositionInTheList = positionInTheList(tokenView.getID(),
								_currentMarkingVector[placeNo]);
						int oldMarking = _currentMarkingVector[placeNo].get(oldMarkingPositionInTheList)
								.getCurrentMarking();
						// tokenView.createIncidenceMatrix(_arcViews,
						// _transitionViews, _placeViews);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see pipe.models.interfaces.IPetriNet#fireTransitionBackwards(pipe.views.
	 * Transition)
	 */
			...
			if (transitionView.isEnabled() && _placeViews != null) {
				...
				for (int placeNo = 0; placeNo < _placeViews.size(); placeNo++) {
					for (MarkingView m : _placeViews.get(placeNo).getCurrentMarkingView()) {
						...
						int markingToBeSubtracted = m.getToken().getIncidenceMatrix().get(placeNo, transitionNo);
	 ...
	 * _transitions.size() * _transitions.size() * _transitions.size(); int
	 * randomTransitionNumber = 0; Transition randomTransition = null; do {
	 * randomTransitionNumber = _randomNumber.nextInt(_transitions.size());
	 * randomTransition = (Transition)_transitions.get(randomTransitionNumber);
	 ...
	 * (non-Javadoc)
	 * 
	 * @see pipe.models.interfaces.IPetriNet#isTransitionEnabled(java.util.
	 * LinkedList, int)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see pipe.models.interfaces.IPetriNet#areTransitionsEnabled(java.util.
	 * LinkedList)
	 */
	public final boolean[] areTransitionsEnabled(LinkedList<MarkingView>[] markings) {
		...
		for (int i = 0; i < transitionCount; i++) {
			...
			for (int j = 0; j < placeCount; j++) {
				...
				// capacities
				if (allTokenClassesEnabled && (_capacityMatrix[j] > 0) && (totalMarkings + totalForwardIncidenceMarkings
						- totalBackwardIncidenceMarkings > _capacityMatrix[j])) { // firing
																					// this
																					// transition
																					// would
																					// break
	...
	public void setEnabledTransitionsBackwards() {
		...
		boolean[] enabledTransitions = getTransitionEnabledStatusArray(this.getTransitionViews(),
				this.getCurrentMarkingVector(), true, this.getCapacityMatrix(), this.numberOfPlaces(),
	...
	public void setEnabledTransitions() {
		...
		boolean[] enabledTransitions = getTransitionEnabledStatusArray(this.getTransitionViews(),
				this.getCurrentMarkingVector(), false, this.getCapacityMatrix(), this.numberOfPlaces(),
				this.numberOfTransitions());
	...
	private boolean[] getTransitionEnabledStatusArray(
			...
			final TransitionView[] transArray, final LinkedList<MarkingView>[] markings,
			boolean backwards, /*
			...
			for (int j = 0; j < placeCount; j++) {
				...
				for (MarkingView m : markings[j]) {
					...
					if (allTokenClassesEnabled && (_capacityMatrix[j] > 0)
							&& (totalMarkings + totalCPlus - totalCMinus > _capacityMatrix[j])) { // firing
																									// this
																									// transition
																									// would
																									// break
	...
	public LinkedList<MarkingView>[] marking() {
		...
		for (int i = 0; i < _placeViews.size(); i++) {
			result[i] = (LinkedList<MarkingView>) Copier
					.deepCopy(((PlaceView) _placeViews.get(i)).getCurrentMarkingView());
		}
		return result;
	}
	...
	private void setMatrixChanged() {
		...
		// bug found: _tokenViews could be null if the first item in nodelist is
		// a place.
	 ...
	 * 
	 * @see
	 * pipe.models.interfaces.IPetriNet#createFromPNML(org.w3c.dom.Document)
	 */
	 ...
	 * 
	 * @see
	 * pipe.models.interfaces.IPetriNet#existsRateParameter(java.lang.String)
	 */
	 ...
	 * 
	 * @see
	 * pipe.models.interfaces.IPetriNet#changeRateParameter(java.lang.String,
	 * java.lang.String)
	 */
	 ...
	 * 
	 * @see
	 * pipe.models.interfaces.IPetriNet#isTangibleState(java.util.LinkedList)
	 */
	 ...
	 * 
	 * @see
	 * pipe.models.interfaces.IPetriNet#checkTransitionIDAvailability(java.lang.
	 * String)
	 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see pipe.models.interfaces.IPetriNet#checkPlaceIDAvailability(java.lang.
	 * String)
	 */
	...
	/*
	 * use to check if structure contain any tagged token or tagged arc, then
	 * the structure needs to be validated before animation
	 */
	/*
	 * (non-Javadoc)
	 * 
	 ...
	 * 
	 * @see
	 * pipe.models.interfaces.IPetriNet#getTransitionByName(java.lang.String)
	 */
	...
	public ArcView createArc(Element inputArcElement) {
		...
		if (inscriptionTempStorage.length() > 0) {
			...
			if (stringArray.length == 1) {
				...
				while (i < stringArray.length) {
					...
					MarkingView markingView = new MarkingView(this.getTokenClassFromID(stringArray[i]),
							stringArray[i + 1]);// Integer.valueOf(stringArray[i
												// + 1])+"");
					Marking marking = new Marking(getTokenClassFromID(stringArray[i]).getModel(), stringArray[i + 1]);// Integer.valueOf(stringArray[i
																														// +
																														// 1])+"");
					weightInput.add(markingView);
					weightModel.add(marking);
		...
		if (type.equals("inhibitor")) {
			tempArcView = new InhibitorArcView((double) aStartx, (double) aStarty, (double) aEndx, (double) aEndy,
					sourceIn, targetIn, weightInput, idInput,
					new InhibitorArc(sourceIn.getModel(), targetIn.getModel()));// ,
																				// weightModel));
		} else {
			tempArcView = new NormalArcView((double) aStartx, (double) aStarty, (double) aEndx, (double) aEndy,
					sourceIn, targetIn, weightInput, idInput, taggedArc,
					new NormalArc(sourceIn.getModel(), targetIn.getModel()));// ,
																				// weightModel));
		}
